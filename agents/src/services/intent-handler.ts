...
export class IntentHandlerService {
  ...
  private readonly quickActionTemplates: Map<string, Action[]> = new Map();
  ...
  async analyzeIntent(message: XMTPMessage): Promise<Intent> {
  ...
  private extractEntities(content: string, intentType: string): Record<string, any> {
  ...
  private generateDescriptionForIntent(intent: Intent, nftVerification: NFTVerificationResult): string {
    const accessLevel = nftVerification.hasAccess ? nftVerification.accessLevel : 'basic';
  ...
  private generateContentForIntent(intent: Intent, nftVerification: NFTVerificationResult): string {
    const accessLevel = nftVerification.hasAccess ? nftVerification.accessLevel : 'basic';
  ...
  private getQuickActionsForLevel(accessLevel: string): Action[] {
