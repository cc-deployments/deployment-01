...
async function testWalletCalls() {
  ...
  const walletCallService = new WalletCallService(config);
  ...
  try {
    ...
    const walletCalls = await walletCallService.createCarStoryTransaction(
  ...
  try {
    const walletCalls = await walletCallService.createMintTransaction(
      "0x1234567890123456789012345678901234567890",
      "premium",
      {
        make: "Ferrari",
  ...
  try {
    const walletCalls = await walletCallService.createCommunityTransaction(
      "0x1234567890123456789012345678901234567890",
      "vote",
      { proposalId: "123", vote: "yes" }
    );
